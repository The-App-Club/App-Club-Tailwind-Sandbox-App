...
const FeatureColumnSelector = ({data, className, isX = true}) => {
  const [featureColumn, setFeatureColumn] = useRecoilState(
    featureColumnSelectorState
  );
  const [selected, setSelected] = useState({
    name: isX ? featureColumn.xColumnName : featureColumn.yColumnName,
  });

  const handleChange = (e) => {
    setFeatureColumn((prevState) => {
      if (isX) {
        return {
          xColumnName: e.name,
          yColumnName: prevState.yColumnName,
        };
      } else {
        return {
          xColumnName: prevState.xColumnName,
          yColumnName: e.name,
        };
      }
    });
